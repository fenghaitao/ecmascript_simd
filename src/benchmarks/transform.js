...
(function () {
  ...
  function simdVertexTransform(n) {
    for (var i = 0; i < n; i++) {
      var xxxx = SIMD.float32x4.shuffle(Vx4[0], SIMD.XXXX);
      var z = SIMD.float32x4.zero();
      z = SIMD.float32x4.add(z, SIMD.float32x4.mul(xxxx, Tx4[0]));
      var yyyy = SIMD.float32x4.shuffle(Vx4[0], SIMD.YYYY);
      z = SIMD.float32x4.add(z, SIMD.float32x4.mul(yyyy, Tx4[1]));
      var zzzz = SIMD.float32x4.shuffle(Vx4[0], SIMD.ZZZZ);
      z = SIMD.float32x4.add(z, SIMD.float32x4.mul(zzzz, Tx4[2]));
      var wwww = SIMD.float32x4.shuffle(Vx4[0], SIMD.WWWW);
      z = SIMD.float32x4.add(z, SIMD.float32x4.mul(wwww, Tx4[3]));
      Outx4[0] = z;
    }
  }
